...
\begin{figure}[h!]
  \caption{Machine N}
\begin{tikzpicture}[>=stealth',shorten >=1pt,auto,node distance=2cm]
  \node[state, accepting] (r1){$r_1$};
  \node[state] [below of=r1] (r2){$r_2$};

  \node[state] [below of=r2] (s1){$s_1$};
  \node[state] [below of=s1] (s2){$s_2$};
  \node[initial, state] [left of=s2] (q0){$q_0$};
  \node[state, accepting] [below of=s2] (s3){$s_3$};
  \node[state] [below of=s3] (s4){$s_4$};
  \path[->]
  (q0)
    edge [bend left] node{$\epsilon$}(s1)
    edge [bend left] node{$\epsilon$}(r1)
  (r1)
    edge [bend left] node{0}(r2)
    edge [loop above] node{1}(r1)
  (r2)
    edge [bend left] node{0}(r1)
    edge [loop right] node{1}(r2)

  (s1)
    edge [loop right] node{0}(s1)
    edge node{1}(s2)
  (s2)
    edge [loop right] node{0}(s2)
    edge node{1}(s3)
  (s3)
    edge [loop right] node{0}(s3)
    edge node{1}(s4)
  (s4)
    edge [loop right] node{0,1}(s4)
  ;
\end{tikzpicture}
\end{figure}
\end{problem}
